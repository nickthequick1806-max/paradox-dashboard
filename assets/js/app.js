(() => {
  ...
  function refTable(headers,rows,opts={}){return `<div class="ref-table-wrap ${opts.className||''}"><table class="ref-table"><thead><tr>${headers.map(h=>`<th>${esc(h)}</th>`).join('')}</tr></thead><tbody>${rows.length?rows.map((r,ri)=>`<tr class="detail-row" data-ref-row="${ri}" data-record-id="${esc(opts.recordIds?.[ri]||state.sectionData?.[state.page]?.rows?.[ri]?.id||'')}">${r.map(c=>`<td>${c}</td>`).join('')}</tr>`).join(''):`<tr><td colspan="${headers.length}">${refEmpty(opts.emptyIcon||'inbox',opts.emptyTitle||'No records found',opts.emptySub||'Records will appear here when available.')}</td></tr>`}</tbody></table></div>`}

  function securityLogsPage(){
    const rows=(state.sectionData?.logs?.rows||[]).map(r=>[esc(window.ParadoxData.date(r.created_at)),esc(r.actor+': '+r.kind)]);
  ...
  function securityLiveViewPage(){
    const list=state.demoSession?(state.live?.liveViews||[]):(state.sectionData?.['live-view']?.rows||[]);
    return `<div class="page ref-page">${refTitle('Security','Live View','Inspect recorded player captures and request fresh evidence','video',`${refStat('Online',players.length)}${refStat('Watching',0,'yellow')}<button class="btn primary" data-action="refresh">${fa('rotate')} Refresh</button>`)}<div class="live-ref-controls"><div class="segmented"><button class="seg">${fa('list')} List</button><button class="seg active">${fa('video')} Grid</button></div><div class="ref-search">${fa('magnifying-glass')}<input placeholder="Filter by name, ID, or license..."></div><button class="btn">Sort: Server ID ${fa('chevron-down')}</button></div><div class="filter-chips">${['Flagged','Possible Cheater','New Player','Cheating Discord','Alt Account','VPN','Starred','Active Streams'].map(x=>`<button disabled title="This classification is not recorded">${x} <b>—</b></button>`).join('')}</div><div class="live-layout-ref"><div class="live-stage-ref">${list.length?list.map(v=>`<div class="stream-card detail-card" data-record-id="${esc(v.id||'')}" data-card-title="Evidence capture"><div class="stream-modal-preview">${fa('image')}</div><div><strong>${esc(v.evidence_id||v.playerName||'Capture')}</strong><span>${esc(v.status||'Not reported')} · ${esc(window.ParadoxData.date(v.created_at))}</span></div></div>`).join(''):refEmpty('video-slash','No recorded captures',state.sectionData?.['live-view']?.error||'Request a screenshot from a player below. Continuous video is not configured.')}</div><aside class="live-player-list-ref"><div class="live-list-head">${fa('users')} <strong>PLAYERS</strong><span>${players.length}</span></div>${refSearch('Search players...')}${players.length?players.map(p=>`<button class="v11-online-player" data-action="screenshotPlayer" data-player="${esc(p[0])}"><span class="avatar">${esc(p[1][0])}</span><span><strong>${esc(p[1])}</strong><small>ID ${esc(p[0])} · Request screenshot</small></span></button>`).join(''):`<div class="live-list-empty">${fa('user-slash')}<span>No players match your filters</span></div>`}</aside></div></div>`
  }
  ...
  function securityLookupPage(){
    return `<div class="page ref-page lookup-ref"><div class="lookup-hero"><h1>${fa('magnifying-glass')} Player Lookup</h1><p>Search the connected server's authorized player records</p><button class="lookup-search v14-lookup-trigger" data-action="openPlayerLookup" type="button">${fa('magnifying-glass')}<span>Search by name, license, FiveM ID, Steam ID, or Discord ID...</span><kbd>Enter</kbd></button><span>${fa('circle-question')} Server permissions and request limits apply</span></div></div>`
  }
  ...
  function securityDetectionsPage(){
    ...
    let normalized=src.map((d,i)=>{const a=Array.isArray(d)?d:null;return {id:a?a[0]:d.id,player:a?a[2]:(d.playerName||d.player||'Unknown'),reason:a?a[1]:(d.reason||d.name||'Detection'),action:a?a[5]:(d.action||d.severity||'WARN'),time:a?.[7]||d.time||'Live'};});
  ...
  function managementBackupsPage(){
    const rows=state.demoSession?[['Auto-backup - 5/15/2026, 2:09:46 AM','4 months ago','May 15, 2026 · 2:09 AM'],['Auto-backup - 4/9/2026, 8:35:00 PM','5 months ago','Apr 9, 2026 · 8:35 PM'],['Auto-backup - 3/30/2026, 12:57:45 AM','6 months ago','Mar 30, 2026 · 12:57 AM'],['Auto-backup - 3/29/2026, 12:26:05 AM','6 months ago','Mar 29, 2026 · 12:26 AM']]:(state.sectionData?.backups?.rows||[]).map(r=>[r.reason,window.ParadoxData.date(r.created_at),'Revision '+r.revision,r.revision]);
    return `<div class="page ref-page">${refTitle('Management','Backups','Create, restore, and manage settings backups','box-archive',`<span class="green-count">${fa('database')} ${rows.length} Backups</span>`)}<div class="backup-actions"><button class="btn primary" data-action="createBackup">${fa('plus')} Create Backup</button><button class="btn">${fa('rotate')} Refresh</button></div><div class="backup-list-ref">${rows.length?rows.map((r,i)=>`<article class="backup-row-ref detail-card" data-card-title="${esc(r[0])}"><div><strong>${fa('robot')} ${esc(r[0])}</strong><span><b>${state.demoSession?'AUTO':'POLICY'}</b> ${fa('clock')} ${r[1]} &nbsp; ${r[2]}</span></div><div><button class="btn blue" data-action="liveRestoreBackup" data-revision="${esc(r[3])}">${fa('rotate-left')} Restore</button><button class="icon-btn" data-action="liveDownloadBackup" data-revision="${esc(r[3])}" title="Download saved policy">${fa('download')}</button></div></article>`).join(''):refEmpty('box-archive','No backups available',state.sectionData?.backups?.error||'Create your first configuration backup.')}</div></div>`
  }
  function managementCdnPage(){
    const data=state.sectionData?.cdn||{},files=data.files||[],fmt=window.ParadoxData;
    return `<div class="page ref-page">${refTitle('Management','CDN Files','Protected evidence stored by the connected server','image')}<section class="cdn-storage-card detail-card" data-card-title="Evidence Storage"><div class="cdn-icon">${fa('database')}</div><div><span>EVIDENCE STORAGE</span><strong>${data.available?'~'+esc(fmt.bytes(data.usedBytes)):'—'} <small>${esc(data.provider||'Loading server storage')}</small></strong><div class="cdn-progress"><i style="width:0%"></i></div><b>${esc(data.error||'Estimated database allocation; no storage quota reported')}</b></div><span>${esc(data.totalCount??'—')} records</span></section><div class="cdn-toolbar"><strong>${fa('hard-drive')} Evidence</strong><span>${fa('chevron-right')}</span><strong>${fa('folder-open')} captures</strong><div class="ref-search">${fa('magnifying-glass')}<input placeholder="Search by filename..."></div><button class="btn dashed" data-action="refresh">${fa('rotate')} Refresh</button><span class="btn">${files.length} latest records</span></div><div class="cdn-grid-ref">${files.length?files.map(f=>`<article class="cdn-file-card detail-card" data-record-id="${esc(f.id)}" data-card-title="${esc(f.id)}"><div class="cdn-thumb"><span>${fa('image')}</span><button class="icon-btn">${fa('ellipsis-vertical')}</button></div><div><strong>${esc(f.id)}</strong><span>${esc(fmt.bytes(f.bytes))} · ${esc(fmt.date(f.created_at))} · ${esc(f.status)}</span></div></article>`).join(''):refEmpty('folder-open','No evidence records',data.error||'Captures requested from a player will appear here.')}</div></div>`;
  }

  function specializedSectionModal(page, card){
    if(!state.demoSession)return window.ParadoxLiveDetail?.(page,card)===true;
  ...
  function closeModal(){state.modalVersion=(state.modalVersion||0)+1;modalLayer.classList.remove('open');modalLayer.setAttribute('aria-hidden','true');setTimeout(()=>{if(!modalLayer.classList.contains('open'))modalLayer.innerHTML=''},180)}
  ...
  async function syncLiveData(showToast=false, renderPage=showToast){
    ...
    try{
      ...
      state.live=Object.assign({connected:true,sections:state.live?.sections||{},storage:state.live?.storage},snap);
  ...
  window.installParadoxLiveDetails({state,esc,fa,openModal,closeModal,toast,render});
