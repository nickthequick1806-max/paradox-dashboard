(() => {
  ...
  function liveRows(key, demoRows=[]){
    const rows=state.live?.sections?.[key] ?? state.live?.[key];
  ...
  function managementPermissionsPage(){
    const tab=state.permissionsTab||'members'; const tabs=[['members','users','Members','1'],['roles','layer-group','Roles','0'],['invite-links','link','Invite Links','0'],['bypasses','shield-halved','Bypasses','']]; let body='';
    if(tab==='members') body=`<div class="section-header-line"><div><h2>Team Members</h2><p>Active members with access to this server's panel.</p></div><button class="btn outline-green" data-action="inviteMember">${fa('envelope')} Invite Member</button></div><div class="member-table"><div class="member-head"><span>USERNAME</span><span>ROLES</span><span>NOTES</span><span></span></div><div class="member-row"><div><strong>FastxFingers</strong><span class="status-online">● Online</span><small>(17092499)</small></div><div><span class="role-blue">All Permissions</span></div><div>—</div><button class="icon-btn">${fa('ellipsis-vertical')}</button></div></div><div class="section-header-line pending-head"><div><h2>Pending Invites</h2><p>Invites that have been sent but not yet accepted.</p></div></div><div class="pending-box">${fa('envelope-open')} No pending invites.</div>`;
    if(tab==='roles') body=`<div class="section-header-line"><div><h2>Permission Roles</h2><p>Reusable permission sets you can assign to members.</p></div><button class="btn outline-green" data-action="createPermissionRole">${fa('plus')} New Role</button></div><div class="permission-empty-row">${fa('layer-group')}<span>No roles yet. Create one to start assigning permissions to members.</span></div>`;
    if(tab==='invite-links') body=`<div class="section-header-line"><div><h2>Invite Links</h2><p>Shareable links that automatically assign roles when accepted.</p></div><button class="btn outline-green" data-action="createInviteLink">${fa('plus')} New Link</button></div><div class="permission-empty-row">${fa('link-slash')}<span>No invite links yet. Create one to let anyone join with a set of roles.</span></div>`;
  ...
  function managementServerDetailsPage(){return `<div class="page ref-page">${refTitle('Management','Server Details','View technical specifications and configuration details for your server','server')}<div class="server-details-grid"><section class="detail-panel detail-card" data-card-title="General Information"><h3>${fa('server')} General Information</h3>${[['Server ID',state.server.serverId||'eplq53'],['Created By','698e5e5c9bb07af610285eb1'],['Locale','en-US'],['Game Type','gta5']].map(x=>`<div class="server-detail-row"><span>${x[0]}</span><strong>${esc(x[1])}</strong></div>`).join('')}</section><section class="detail-panel detail-card" data-card-title="System Information"><h3>${fa('microchip')} System Information</h3>${[['Operating System','Linux'],['Build Number','28108'],['Version','Premium · 4.4.7'],['Product Type','Premium'],['Custom Artifacts','Enabled']].map((x,i)=>`<div class="server-detail-row"><span>${x[0]}</span><strong class="${i>2?'status-green':''}">${x[1]}</strong></div>`).join('')}</section><section class="detail-panel detail-card" data-card-title="Storage & Plan"><h3>${fa('database')} Storage & Plan</h3>${[['Max Storage','6.00 GB'],['CDN Usage','0.03 GB (42 files)'],['Storage Used','0.03 / 6.00 GB  · 0.6%'],['Plan Reference','tbx-6854226a80214-cf2ead']].map(x=>`<div class="server-detail-row"><span>${x[0]}</span><strong>${x[1]}</strong></div>`).join('')}</section></div></div>`}
  ...
  function toggleRow(icon,title,sub,on=true,action='v11Toggle'){return `<div class="v11-toggle-row">${fa(icon)}<div><strong>${title}</strong><span>${sub}</span></div><button class="switch ${on?'on':''}" data-action="${action}" aria-pressed="${on}"></button></div>`}
  ...
  function generalRulesPage(){
    ...
    return `<div class="page ref-page v11-page">${v11Title('Configuration','General AntiCheat Rules','Manage and control how cheaters are handled in your server.','shield-halved')}
      ...
      <section class="v11-detector-grid">${detectors.map((x,i)=>`<div class="v11-detector-card"><div><strong>${x} ${fa('circle-question')}</strong><span>Detects ${x.replace('Anti ','').toLowerCase()} behavior...</span></div><button class="switch ${(i===6||i===9||i===26)?'':'on'}" data-action="v11Toggle"></button><button class="icon-btn" data-action="detectorSettings" data-name="${esc(x)}">${fa('gear')}</button></div>`).join('')}</section>
    </div>`;
  }
  ...
  function playerModal(name, tab='details'){
    const p=players.find(x=>x[1]===name||String(x[0])===String(name))||players[0]||['—',name||'Unknown','license:unavailable','Unknown','—',0,'Offline'];
    ...
    const ids=raw.identifiers&&typeof raw.identifiers==='object'?raw.identifiers:{discord:raw.discord||'1298876575399411787',fivem:raw.fivem||'16635112',license2:raw.license2||p[2]?.replace(/^license:/,'')||'0098bf...',rockstar:raw.rockstar||'d057cf1c0027804f0f99d9c064a24bd8125e0eee'};
    ...
    const serverNames=['Paradox City RP','Classic Roleplay 18+','Palm Beach','8Trap RP','New Era','The Streets LA']; const servers=(raw.servers||serverNames).slice(0,8).map((sv,i)=>{const n=typeof sv==='string'?sv:(sv.name||'FiveM Server');return `<div class="v12-joined-server"><span class="v12-server-thumb">${fa('server')}</span><div><strong>${esc(n)}</strong><small>${i===0?'Current server':'Join date unknown'}</small></div><b>● ${i===0?'Online':'Offline'}</b></div>`}).join('');
    ...
    let right='';
    if(tab==='details') right=`<div class="v12-player-profile"><span class="v12-offline">● ${esc(raw.status||p[6]||'OFFLINE')}</span><h2>${esc(p[1])}</h2>${Number(p[5])>40?'<span class="v12-risk-badge">Low Playtime</span>':''}</div><div class="v12-player-kpis"><div><span>${fa('clock')} FIRST SEEN</span><strong>${esc(raw.firstSeen||'5 months ago')}</strong></div><div><span>${fa('right-to-bracket')} LAST JOINED</span><strong>${esc(raw.lastJoined||'13 days ago')}</strong></div><div><span>${fa('server')} THIS SERVER</span><strong>${esc(raw.serverPlaytime||'0m')}</strong></div><div><span>${fa('globe')} TOTAL PLAYTIME</span><strong>${esc(raw.totalPlaytime||'5d 23h')}</strong></div></div><div class="v12-block-title">${fa('server')} SERVERS JOINED <b>${raw.servers?.length||serverNames.length}</b></div><div class="v12-server-grid">${servers}</div>`;
  ...
  function showLogin(){
    ...
    $('#loginForm',authLayer)?.addEventListener('submit',async e=>{ e.preventDefault(); await beginLogin($('#loginEmail').value,$('#loginPassword').value); });
  }
  ...
  async function syncLiveData(showToast=false, renderPage=showToast){
    ...
    try{
      const snap=await window.ParadoxAPI.getSnapshot();
      ...
      state.live=Object.assign({connected:true},snap);
      ...
      if(Array.isArray(snap.detections)) detections=snap.detections.map(d=>[d.id||'DET',d.name||d.label||'Detection',d.player||d.playerName||'Unknown',d.key||d.detection||'security.signal',String(d.confidence??0),d.severity||'Medium',d.action||'Observed',d.time||d.createdAt||'Live']); else detections=[];
  ...
  document.addEventListener('click', async e=>{
    ...
    if(action==='dashboardAccess'){closeDropdown();await dashboardAccessModal();return;}
    ...
    if(action==='completeFirstSetup'){
      ...
      try{
        ...
        state.setupComplete=true;state.server.linked=true;localStorage.setItem('pa_setup_complete','1');authLayer.classList.remove('open');authLayer.innerHTML='';document.body.classList.remove('auth-active');await syncLiveData(true);render();
  ...
  const production=window.createParadoxProduction({state,esc,header,render:()=>render(),openModal,toast});
  const previewPage=genericPage;
  genericPage=function(key){return state.demoSession?previewPage(key):production.page(key,...(pageMeta[key]||['Dashboard','Server operations']));};
  window.ParadoxRefresh=()=>syncLiveData(false,false);
  document.addEventListener('click',e=>{
    if(state.demoSession)return;
    const sw=e.target.closest('.switch');
    if(sw){e.preventDefault();e.stopImmediatePropagation();toast('Use server policy','Change runtime settings in Configuration.','bad');return;}
    const a=e.target.closest('[data-action]');if(!a)return;
    const safe=new Set(['closeModal','logout','accountSettings','accountTab','saveAccount','settingsTab','profile','notifications','notificationRead','markAllRead','openDocs','refresh','closeDropdown','openSettings','accountSettingsTab','saveAccountSettings','globalSearch','runGlobalSearch','openPlayerLookup','runPlayerLookupModal','appearanceSettings','saveAccent','saveAppearance','resetAccent','connectedServers','dashboardAccess','disconnectServer']);
    if(!safe.has(a.dataset.action)){e.preventDefault();e.stopImmediatePropagation();toast('Use live controls','Open the relevant dashboard page to use its server-connected controls.','bad');}
  },true);
  ...
  if(!state.authenticated){
    ...
    setInterval(()=>syncLiveData(false,false),Math.max(2500,Number(state.server.syncInterval||5000)));
  }
})();
